...
if (isset($_POST['SimpanEdit'])) {
  ...
  $queryuser = mysqli_query($conn, "SELECT * FROM pengguna WHERE id='" . $_COOKIE['id'] . "'");
  $cariuser = mysqli_fetch_assoc($queryuser);

  if ($cariuser['password'] == $pass) {
    if ($cariuser) {
      $cekDataUpdate =  mysqli_query($conn, "UPDATE pengguna SET nama='" . $uname . "',
            username ='" . $user . "',
            email='" . $email . "',
            tlpn ='" . $telp . "',
            update_at = '" . $currdate . "'
            WHERE id_pengguna='" . $_COOKIE['id'] . "'") or die(mysqli_connect_error());
      if ($cekDataUpdate) {
...
if (isset($_POST['UpdatePass'])) {
  ...
  $currdate = date('Y-m-d H:i:s');
  $querypass = mysqli_query($conn, "SELECT * FROM pengguna  WHERE id_pengguna='" . $_COOKIE['id'] . "'");
  ...
  if ($pass1 == $caripass['password']) {
    if ($caripass) {
      ...
      if ($pass2 == $pass3) {
        $cekPass =  mysqli_query($conn, "UPDATE pengguna SET password='$pass3',
                update_at = '$currdate'
                    WHERE id_pengguna='" . $_COOKIE['id'] . "'") or die(mysqli_connect_error());
        if ($cekPass) {
    ...
    <div class="media align-items-end profile-header">
      <form method="POST" action="proses-logo.php" enctype="multipart/form-data">
        <div class="profile mr-3">
          <label for="logo"><img src="../uploads/profil/<?= $d['logo']; ?>" alt="logo" class="img-cover-profile rounded mb-2 img-thumbnail"></label>
          <input type="file" class="d-none" id="logo" onchange="form.submit()" name="file" />
          <input type="hidden" value="<?= $d['profil']; ?>" class="d-none" name="foto2" />
        </div>
      </form>
      <div class="media-body mb-5 text-white">
        <h4 class="mt-0 mb-0"><?php echo $d['nama']; ?></h4>
        <p class="small mb-4"><?= $d['level'] ?> <?= $d['level'] != 'Administrator' ? 'Team' : '' ?></p>
      </div>
    </div>
  </div>
        ...
        <form method="post">
          <div class="row">
            ...
            <div class="col-sm-6 col-md-6 mb-2">
              ...
              <input type="number" name="telepon" class="form-control" value="<?= $d['tlpn']; ?>" id="aplikasi" placeholder="Telepon" required>
            </div>
