...
if (isset($_POST['SimpanEdit'])) {
    ...
    $queryuser = mysqli_query($conn, "SELECT * FROM tb_pengguna WHERE id='" . $_COOKIE['id'] . "'");
    $cariuser = mysqli_fetch_assoc($queryuser);

    if ($cariuser['password'] == $pass) {
        if ($cariuser) {
            $cekDataUpdate =  mysqli_query($conn, "UPDATE tb_pengguna SET nama='" . $uname . "',
            username ='" . $user . "',
            email='" . $email . "',
            tlpn ='" . $telp . "',
            update_at = '" . $currdate . "'
            WHERE id='" . $_COOKIE['id'] . "'") or die(mysqli_connect_error());
            if ($cekDataUpdate) {
...
if (isset($_POST['UpdatePass'])) {
    ...
    $currdate = date('Y-m-d H:i:s');
    $querypass = mysqli_query($conn, "SELECT * FROM tb_pengguna  WHERE id='" . $_COOKIE['id'] . "'");
    ...
    if ($pass1 == $caripass['password']) {
        if ($caripass) {
            ...
            if ($pass2 == $pass3) {
                $cekPass =  mysqli_query($conn, "UPDATE tb_pengguna SET password='$pass3',
                update_at = '$currdate'
                    WHERE id='" . $_COOKIE['id'] . "'") or die(mysqli_connect_error());
                if ($cekPass) {
        ...
        <div class="media align-items-end profile-header">
            <form method="POST" action="proses-logo.php" enctype="multipart/form-data">
                <div class="profile mr-3">
                    ...
                    <input type="hidden" value="<?= $d['logo']; ?>" class="d-none" name="foto2" />
                </div>
            </form>
